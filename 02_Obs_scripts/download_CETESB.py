...
#%%
cetesb_stations = pd.read_csv('../01_data/stations_hc.csv')
                              #encoding = "ISO-8859-1")
...
for c,n in zip(cetesb_stations.code,cetesb_stations.name):
    ...
    qr.all_photo(cetesb_login, cetesb_password, start_date, end_date, c, csv_photo=False)
    print('Downloading meteo ' + n + ' Station')
    qr.all_met(cetesb_login, cetesb_password,  start_date, end_date, c, csv_met=False)
